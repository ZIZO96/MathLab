...
string XOR(string input1, string input2)
{
	...
	string output = "";
	if ((input1.length() == input2.length()) && ((input1 != "") || (input2 != "")))
	{
		
		for (unsigned int i = 0; i < input1.length(); i++)
		{
			output += ((input1[i] - '0') ^ (input2[i] - '0')) + '0';
		}
	...
	{
		cout << "Input error in the XOR Function in the call number "<< callCount <<  "!" << endl;
		callCount++;
		return output;
	}
}
...
int main()
{
	...
	//string sentMessage = sent_msg(message, generatorFunction);
	/*if (VERIFIER(sentMessage, generatorFunction))
	{
